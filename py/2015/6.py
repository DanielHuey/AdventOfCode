...
def stp(string:str):
        pt = string.split(',')
        return Point(int(pt[0]),int(pt[1]))

# part 1
r""" 
lights = np.array([[0 for i in range(1000)] for j in range(1000)],dtype=bool)
print(np.shape(lights))
# Commands
def on(a:Point,b:Point):
    for i in range(a.x,b.x+1):
        for j in range(a.y,b.y+1):
            lights[i,j] = True

def off(a:Point,b:Point):
    for i in range(a.x,b.x+1):
        for j in range(a.y,b.y+1):
            lights[i,j] = False

def tog(a:Point,b:Point):
    for i in range(a.x,b.x+1):
        for j in range(a.y,b.y+1):
            lights[i,j] = not lights[i,j]

def compile(string:str):
    match = re.findall(r"\b\d+,\d+",string)
    if string.startswith("turn on"):
        on(stp(match[0]),stp(match[1]))

    elif string.startswith("turn off"):
        off(stp(match[0]),stp(match[1]))
    
    elif string.startswith("toggle"):
        tog(stp(match[0]),stp(match[1]))

for cmd in inputs.split("\n"):
    compile(cmd)

# on(stp('0,0'),stp('2,2')) # using this to test for output 9

print(np.count_nonzero(lights)) """

# part 2
lights2 = np.array([[0 for i in range(1000)] for j in range(1000)],dtype=int)

def on2(a:Point,b:Point):
    for i in range(a.x,b.x+1):
        for j in range(a.y,b.y+1):
            lights2[i,j] += 1

def off2(a:Point,b:Point):
    for i in range(a.x,b.x+1):
        for j in range(a.y,b.y+1):
            lights2[i,j] -= 1 if lights2[i,j] > 0 else 0 # *to a minimum of 0

def tog2(a:Point,b:Point):
    for i in range(a.x,b.x+1):
        for j in range(a.y,b.y+1):
            lights2[i,j] += 2

def compile2(string:str):
    match = re.findall(r"\b\d+,\d+",string)
    if string.startswith("turn on"):
        on2(stp(match[0]),stp(match[1]))

    elif string.startswith("turn off"):
        off2(stp(match[0]),stp(match[1]))
    
    elif string.startswith("toggle"):
        tog2(stp(match[0]),stp(match[1]))
